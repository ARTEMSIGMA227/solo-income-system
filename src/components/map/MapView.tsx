...
export function MapView({ mapHook }: MapViewProps) {
  ...
  TERRITORIES.forEach((ter) => {
    ter.connections.forEach((conn) => {
      const pairKey = [ter.id, conn.targetId].sort().join('--');
  ...
  const activeNameResolved = activeTerritory
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 2 }}>
        ...
        >
          {[
            { color: '#4a4540', label: t.mapView.legend.locked, symbol: '▪' },
            { color: '#8a8070', label: t.mapView.legend.foggy, symbol: '◌' },
            { color: '#5d8a3c', label: t.mapView.legend.available, symbol: '◆' },
            { color: '#c4880d', label: t.mapView.legend.inBattle, symbol: '◈' },
            { color: '#d4a830', label: t.mapView.legend.captured, symbol: '★' },
          ].map((item) => (
            <div
              key={item.label}
        ...
        {activeTerritory && (
          ...
          >
            ...
            >
              ⚔️ {activeTerritory.icon} {activeNameResolved}
            </span>
          </div>
        )}
