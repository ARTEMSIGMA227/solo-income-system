...
export default function BossesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      let { data: bossesData } = await supabase.from('bosses').select('*').eq('user_id', user.id).order('boss_type');
      ...
      if (activeBosses.length === 0) {
        ...
        await supabase.from('bosses').insert([
          {
            user_id: user.id, title: 'Страж рутины', boss_type: 'weekly',
            description: 'Выполни недельный план действий и дохода!',
            requirements: [{ metric: 'actions', target: 150 }, { metric: 'income', target: 25000 }],
            xp_reward: 500, deadline: weekDeadlineStr,
          },
          {
            user_id: user.id, title: 'Теневой Монарх', boss_type: 'monthly',
            description: 'Достигни месячной цели дохода и действий!',
            requirements: [{ metric: 'actions', target: 600 }, { metric: 'income', target: 150000 }, { metric: 'sales', target: 10 }],
            xp_reward: 2000, deadline: monthDeadlineStr,
          },
        ]);

        const { data: refreshed } = await supabase.from('bosses').select('*').eq('user_id', user.id).order('boss_type');
  ...
  function getBossProgress(boss: Boss, currentStats: Stats | null) {
    ...
    return boss.requirements.map((req) => {
      ...
      // Apply boss damage bonus — effectively reduces target
      const effectiveTarget = dmgBonus > 0
  ...
  function getMetricLabel(metric: string) {
    switch (metric) {
      case 'actions': return 'Действия'; case 'clients': return 'Клиенты';
      case 'income': return 'Доход'; case 'sales': return 'Продажи'; default: return metric;
    }
  }
  ...
  function getDaysLeft(deadline: string) {
    if (!now) return 0;
    return Math.max(0, Math.ceil((new Date(deadline).getTime() - now) / (1000 * 60 * 60 * 24)));
  }
  ...
  async function tryDefeatBoss(boss: Boss) {
    ...
    if (!allDone) { toast.error('Ещё не все требования выполнены!'); return; }

    const supabase = createClient();

    // Apply boss damage bonus to XP reward too
    const baseReward = boss.xp_reward;
    const finalReward = applyBossDamageBonus(baseReward, skillEffects);

    await supabase.from('bosses').update({ is_defeated: true, defeated_at: new Date().toISOString() }).eq('id', boss.id);

    const today = new Date().toLocaleDateString('en-CA', { timeZone: 'Europe/Berlin' });
    await supabase.from('xp_events').insert({
      user_id: userId, event_type: 'boss_killed', xp_amount: finalReward,
      description: `Босс повержен: ${boss.title}${finalReward > baseReward ? ` (бонус навыков: +${finalReward - baseReward} XP)` : ''}`,
      event_date: today,
    });
    ...
    await supabase.from('stats').update({
      ...
      total_xp_earned: newTotalEarned, updated_at: new Date().toISOString(),
    }).eq('user_id', userId);
    ...
    const bonusText = finalReward > baseReward ? ` (🧬 +${finalReward - baseReward} бонус)` : '';
    toast.success(`🎉 БОСС ПОВЕРЖЕН! +${finalReward} XP${bonusText}`);
  }
      ...
      <div key={boss.id} style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '4px' }}>{boss.description}</div>
            {dmgBonus > 0 && !boss.is_defeated && (
              <div style={{ fontSize: '10px', color: '#a78bfa', marginTop: '2px' }}>
                🧬 Бонус урона: -{dmgBonus}% требования
              </div>
            )}
          </div>
          <div style={{
          ...
          <div key={i} style={{ marginBottom: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '4px' }}>
              ...
              <span style={{ color: p.percent >= 100 ? '#22c55e' : '#a78bfa' }}>
                {formatMetricValue(p.metric, p.current)} / {formatMetricValue(p.metric, p.effectiveTarget)}
              </span>
            </div>
            <div style={{ width: '100%', height: '8px', backgroundColor: '#16161f', borderRadius: '4px', overflow: 'hidden' }}>
              <div style={{ width: `${p.percent}%`, height: '100%', borderRadius: '4px', backgroundColor: p.percent >= 100 ? '#22c55e' : '#7c3aed' }} />
            </div>
          </div>
        ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
          <span style={{ fontSize: '14px', color: '#f59e0b', fontWeight: 600 }}>
            ...
            {dmgBonus > 0 && <span style={{ fontSize: '10px', color: '#a78bfa', marginLeft: '4px' }}>(+бонус)</span>}
          </span>
          {!boss.is_defeated && (
            <button onClick={() => tryDefeatBoss(boss)} style={{
              padding: '10px 20px',
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0a0f', color: '#e2e8f0', padding: '16px', maxWidth: '600px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '16px' }}>👹 Боссы</h1>
      ...
      {weekly.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ fontSize: '13px', fontWeight: 600, color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>⚡ Недельные</div>
      ...
      {monthly.length > 0 && (
        <div>
          <div style={{ fontSize: '13px', fontWeight: 600, color: '#f59e0b', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>👑 Месячные</div>
